package ca.cshawi.myinventory.items.items
...
class ItemAdapter(val items: MutableList<Item>) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.changedQuantity.text =
            (if (item.changedQuantity > 0) "+" else "") + item.changedQuantity
        holder.itemCode.text = "${item.barCode}"

        holder.increaseButton.setOnClickListener {
            item.changedQuantity++;

            holder.changedQuantity.setTextColor(
        ...
        holder.decrease_button.setOnClickListener {
            item.changedQuantity =
                if (item.currentAmount + item.changedQuantity - 1 >= 0) item.changedQuantity - 1 else item.changedQuantity
