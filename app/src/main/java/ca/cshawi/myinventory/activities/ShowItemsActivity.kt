...
import ca.cshawi.myinventory.adapters.ItemAdapter
...
class ShowItemsActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        speedDial.addActionItem(SpeedDialActionItem.Builder(R.id.fab_new, R.drawable.baseline_add_white_36).setFabBackgroundColor(ResourcesCompat.getColor(resources, R.color.red, theme)).setLabel("Ajouter un objet").create())

        speedDial.addActionItem(SpeedDialActionItem.Builder(R.id.fab_save, R.drawable.round_save_white_36).setFabBackgroundColor(ResourcesCompat.getColor(resources, R.color.green, theme)).setLabel("Enregistrer et Quitter").create())
        ...
        speedDial.setOnActionSelectedListener(SpeedDialView.OnActionSelectedListener { actionItem ->
            when (actionItem.id) {
                ...
                R.id.fab_save -> {
                    ...
                    APIService.INSTANCE.updateItems(UpdateItemsRequest(box.id, box.items).toString()).enqueue(object : Callback<ActionResponse> {
        ...
        findViewById<TextView>(R.id.item_box_title).text = box.title
        ...
        swipeHelper = object : SwipeHelper(this, recyclerView) {
            override fun instantiateUnderlayButton(viewHolder: RecyclerView.ViewHolder, underlayButtons: MutableList<UnderlayButton>) {
                underlayButtons.add(UnderlayButton("Supprimer", 0, Color.parseColor("#FF3C30"), UnderlayButtonClickListener {
                    Snackbar.make(viewHolder.itemView, "Ëtes-vous certain de vouloir supprimer cet objet?", Snackbar.LENGTH_LONG).setCallback(object : Snackbar.Callback() {
                        override fun onDismissed(transientBottomBar: Snackbar?, event: Int) {
                            if (event != DISMISS_EVENT_TIMEOUT) return

                            APIService.INSTANCE.delItem("/boxes/${box.id}/item/${adapter.items[it].id}").enqueue(object : Callback<ActionResponse> {
                                override fun onFailure(call: Call<ActionResponse>, t: Throwable) {
                                    t.printStackTrace()
                                }
